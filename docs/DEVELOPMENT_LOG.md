...
## 18 September: adversarial audit and its fixes

Three independent audits went over the rubric compliance, the submission mechanics and the code. The worst finding was in the video: the old startup footage came from a recording console whose fine print read "Data is invented", meaning the sample payloads were synthetic, but reading as something far worse in an academic submission. That footage is gone; the startup section now shows the documented start commands, their real output and the /health response naming the MySQL driver and the deadline. The audits also caught a layout regression I had introduced with the delete control, which made the saved-experiments rows overlap, and a set of documentation contradictions: the README denied that delete existed, the requirements doc put it out of scope, and three documents disagreed about the test count. The API's server-side rejection, which the brief asks to see, was never on screen and now is, with its real field errors.

## 19 September: effectively-once retries

The browser now mints one id per click and sends it as both the exchange id and an idempotency key. The coordinator replays a settled run for a repeated key without calling the dependency; an unsettled attempt is sent again, and the dependency executes once per key, including when the retry arrives mid-flight. A real exchange against the lecturer schema is on screen in the demonstration: timeout at 406 ms, the dependency finishing a second later, and the retry replayed in 17 ms as a success. A GitHub Actions workflow now runs the full gate and the live adapter test against a MySQL 8.4 service on every push. The suite is 68 tests.

## 20 September: protocol boundaries and final report revision
