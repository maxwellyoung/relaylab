...
import { mySqlSslFromEnvironment } from "../server/dist/database.js";
...
const runsSql = `
  SELECT r.id AS run, r.experiment_id AS experiment, e.name, r.outcome,
         r.http_status AS http, r.duration_ms AS ms, r.created_at
...
async function query() {
  ...
  if (driver === "mysql") {
    ...
    try {
      ...
      return { label: "MySQL", counts, runs };
    } finally {
...
const { label, counts, runs } = await query();
